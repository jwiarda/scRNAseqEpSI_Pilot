# Title: 13 - Gene Set Enrichment Analysis w/ Porcine SI Epithelium
# Date: 2022April07
# Author: Jayne Wiarda
...
write_xlsx(AUCs, '/home/Jayne.Wiarda/scRNAseqEpSI/AUC/EpSI_AUCscores.xlsx')
...
ep.integrated <- AddMetaData(object = ep.integrated, 
                  metadata = c(AUCs))

VlnPlot(ep.integrated, 
        features = colnames(AUCs[1:6]),
...
b4 <- subset (AUCs, celltype == 'BEST4 enterocyte')
b4 <- b4 %>%
  pivot_longer(c(colnames(b4[1:6])), names_to = 'geneSet', values_to = 'AUC')
b4$geneSet <- factor(b4$geneSet,levels = colnames(AUCs[1:6]))
ggplot(b4, aes(x = geneSet, y = AUC, fill = geneSet)) +
  geom_violin(scale = 'width')  +
  geom_jitter(shape=16, position=position_jitter(0.35), size = 0.6) +
  theme_classic()+
  theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1)) +
  scale_fill_manual(values = c('burlywood4', 'goldenrod1', 'darkorange2', 'chartreuse3', 'turquoise4', 'darkmagenta')) +
  geom_boxplot(width = 0.15, fill = 'white', col = 'black', lwd = 0.5, outlier.shape = NA, coef = 0) +
  stat_summary(fun.y = mean, geom='point', size = 2, colour = "red") # red dot shows mean; box shows IQR


# Plot on heatmap
AUCs$celltype <- ep.integrated$celltype
AUCs2 <- AUCs %>% 
  pivot_longer(c(colnames(AUCs[1:6])), names_to = "geneSet", values_to = "AUC")

AUCs2$geneSet <- factor(AUCs2$geneSet,levels = colnames(AUCs[1:6]))
AUCs2$celltype <- factor(AUCs2$celltype,levels = levels(ep.integrated$celltype))

ggplot(AUCs2, aes(geneSet, celltype, fill = AUC))+ 
  geom_tile()+
  scale_fill_viridis(option = 'viridis', limits = c(0, .5), oob=squish) +
  theme_classic()+
  theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))
