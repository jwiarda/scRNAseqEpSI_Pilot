# Title: 08 - Reference Mapping and Prediction to Human Small Intestine
# Date: 2021Sept26
# Author: Jayne Wiarda
...
ref <- as.data.frame(read.table('/home/Jayne.Wiarda/scRNAseqEpSI/RefData/Haber2017_regional_sampling_UMIcounts.txt'))
rownames(ref) <- ref$GENE
ref = subset(ref, select = -c(GENE))
ref <- as.sparse(ref) # make into sparse matrix

# Filter to only one-to-one gene orthologs:
refgenes <- rownames(ref) # extract gene names from reference dataset
reforthos <- intersect(refgenes, orthoGenes$Mouse.gene.name) # find which gene names from reference are also one-to-one orthologs
#length(reforthos) # how many genes are orthologs?

# Create new Seurat object:
meta <- as.data.frame(colnames(ref))
colnames(meta) <- 'cellID'
rownames(meta) <- meta$cellID
meta <- meta %>% 
  separate(cellID, into = c("cellID", "annotation"), sep="_(?=[^_]+$)") # create a column with cell annotation
meta <- meta %>% 
  separate(cellID, into = c("cellID", "animalID"), sep="_(?=[^_]+$)") # create a column with animal ID
meta <- meta %>% 
  separate(cellID, into = c("barcode", "tissue"), sep="_(?=[^_]+$)") # create a column with tissue ID; remaining information is our cell barcode ID now
meta$SampleID <- paste(meta$animalID, meta$tissue, sep = '_')
...
ref.list <- SplitObject(ref, split.by = "SampleID") # split into the original samples that were processed for scRNA-seq
...
DimPlot(ref, 
        group.by = "tissue") # plot by tissue ID
DimPlot(ref, 
        group.by = "animalID") # plot by animal ID
DimPlot(ref, 
        group.by = "annotation") # see cell type annotations

# Save reference as an .h5seurat object:
SaveH5Seurat(ref, filename = "/home/Jayne.Wiarda/scRNAseqEpSI/RefData/MurineSIEpCells_Haber2017_GeneCellFiltered.h5Seurat")
...
rownames(querycounts) <- orthoGenes$Mouse.gene.name # change pig genes to human gene names
...
saveRDS(query, '/home/Jayne.Wiarda/scRNAseqEpSI/MappingPrediction/MurineComp_EpOnly_query.rds')
...
write_xlsx(MappingScores, '/home/Jayne.Wiarda/scRNAseqEpSI/MappingPrediction/MmSI_EpOnly_Haber2017_MappingScores.xlsx')
write_xlsx(CellTypePredictions, '/home/Jayne.Wiarda/scRNAseqEpSI/MappingPrediction/MmSI_EpOnly_Haber2017_CellTypePredictions.xlsx')
